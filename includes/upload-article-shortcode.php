...
//	    $args['edit_date'] = true;
    ...
    update_post_meta( $post_id, 'started_running_at', time() );
...
//	$message = "";
//	if ( !$is_saturday && !$contains_video && !$is_future_post ) {
	if ( !$is_saturday && !$contains_video && !$is_future_post && !current_user_can('administrator') ) {
        wp_update_post( array(
			'ID' => $post_id,
			'post_status' => 'running',
		) );
...
//    wp_enqueue_script( 'upload-article-multiselect-js', 'https://cdnjs.cloudflare.com/ajax/libs/selectize.js/0.15.2/js/selectize.min.js' );
    ...
    wp_enqueue_style( 'upload-article-shortcode', get_template_directory_uri() . '-child/css/upload-article-shortcode.css', array(), '6.1.10027' );
